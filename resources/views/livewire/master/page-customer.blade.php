...
<div class="py-2">
    <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
        <div class="bg-white overflow-hidden shadow-xl sm:rounded-lg">
            ...
            <div class="p-4">
                            ...
                            <div class="rounded-md shadow-sm -space-y-px">
                                <div class="grid gap-6">
                                    <div class="col-span-12">
                                        ...
                                        <input type="text" name="customer" wire:model='customer'
                                            class="mt-1 focus:ring-indigo-500 focus:border-indigo-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md"
                                            required auto-complete="current-kodeProduk" placeholder="customer"
                                            value="{{ $customer }}">
                                        @error('customer')
                                    ...
                                    <div class="col-span-12">
                                        <label for="email_address" class="block text-sm font-medium text-gray-700">No
                                            Telpon</label>
                                        <input type="text" name="no_telpon" wire:model='no_telpon' id="no_telpon"
                                            required autocomplete="current-tgl_Produk"
                                            class="mt-1 focus:ring-indigo-500 focus:border-indigo-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md"
                                            value="{{ $no_telpon }}" placeholder="no_telpon">
                                        @error('no_telpon')
                                            <span class="text-red-500">{{ $message }}</span>
                                        @enderror
                                    </div>
                                    <div class="col-span-12">
                                        ...
                                        <input type="text" name="alamat" wire:model='alamat' id="alamat" required
                                            autocomplete="current-tgl_Produk"
                                            class="mt-1 focus:ring-indigo-500 focus:border-indigo-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md"
                                            value="{{ $alamat }}" placeholder="Produk">
                                        @error('alamat')
                                            <span class="text-red-500">{{ $message }}</span>
                                        @enderror
                                    </div>
                ...
                <span x-data="{ userSaved: @entangle('userSaved') }">
                    <x-jet-action-message class="mr-3 text-red-600" on="saved" x-show='userSaved'>
                        <div class="bg-red-100 border border-red-400 w-40 text-red-700 px-4 py-3 rounded relative"
                            role="alert">
            ...
            {{-- <livewire:restore-all model="App\Model\customer" /> --}}
            {{-- <livewire:customer-table /> --}}
        </div>
    </div>
</div>

    ...
    <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
        <div class="bg-white overflow-hidden shadow-xl sm:rounded-lg">
            <div class="p-4">
                ...
                <!-- Add Item Modal -->
                <x-jet-dialog-modal wire:model="addItem" maxWidth='lg'>
                    <form>
                        @csrf
                        <x-slot name="title">
                        ...
                        <x-slot name="content">
                            <div>
                                <x-jet-label for="" value="{{ __('Kode-Barang Masuk') }}" />
                                <div class="flex flex-row">
                                    <div>
                                        <x-jet-input name='KBM' class="block mt-2 w-full" type='text' wire:model='KBM' />
                                        @error('KBM')
                                            <span class="text-red-500 text-xs italic">{{ $message }}</span>
                                        @enderror
                                    </div>
                                    <x-jet-button type='button' class="block" wire:click='generateKode'>
                                        Buat Kode
                                    </x-jet-button>
                                </div>
                            </div>
                                    ...
                                    <select id="supplier"
                                        ...
                                        @if (!empty($getBB))
                                            @foreach ($getBB as $bbs)
                                                <option value='{{ $bbs->default_stock_id }}'>{{ $bbs->default_stock->bahan_baku }}</option>
                                            @endforeach
                            ...
                            <div>
                                ...
                                <x-jet-input name='KBM' class="block mt-2 w-full" wire:keyup='sub_total'
                            ...
                            <div>
                                <x-jet-label for="" value="{{ __('Tanggal Pembelian') }}" />
                                <x-jet-input name='KBM' class="block mt-2 w-full" type='date' wire:model='tgl' />
                                @error('tgl')
                                    <span class="text-red-500 text-xs italic">{{ $message }}</span>
                                @enderror
                            </div>
